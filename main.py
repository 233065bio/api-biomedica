...
# ─────────────────────────────────────────────
# CALIBRACIÓN: estado en memoria (sin BD)
# ─────────────────────────────────────────────
_calib = {
    "activa":  False,
    "inicio":  None,
    "buffer":  {}   # tipo_senal -> [{"t": ms_epoch, "v": float}]
}
_CALIB_MAX      = 600   # máximo de muestras almacenadas por tipo
_CALIB_DURACION = 60    # segundos
...
class DatosCalibracionModel(BaseModel):
    spo2: float
    ecg: float
    acce_z: float
    flujo: float
...
# ─────────────────────────────────────────────
# CALIBRACIÓN (sin BD, buffer en memoria)
# ─────────────────────────────────────────────
@app.post("/calibracion/iniciar")
def iniciar_calibracion():
    _calib["activa"] = True
    _calib["inicio"] = _time.time()
    _calib["buffer"] = {}
    return {"status": "ok", "inicio": _calib["inicio"]}

@app.post("/calibracion/detener")
def detener_calibracion():
    _calib["activa"] = False
    return {"status": "ok"}

@app.post("/calibracion/datos")
def recibir_datos_calibracion(datos: DatosCalibracionModel):
    """Endpoint que llama el ESP32 durante la calibración (no guarda en BD)."""
    if not _calib["activa"]:
        return {"status": "idle"}

    now_ms = datos.timestamp_ms if datos.timestamp_ms else int(_time.time() * 1000)

    for tipo, valor in [("ecg", datos.ecg), ("spo2", datos.spo2),
                         ("acce_z", datos.acce_z), ("flujo", datos.flujo)]:
        if tipo not in _calib["buffer"]:
            _calib["buffer"][tipo] = []
        _calib["buffer"][tipo].append({"t": now_ms, "v": round(float(valor), 4)})
        if len(_calib["buffer"][tipo]) > _CALIB_MAX:
            _calib["buffer"][tipo] = _calib["buffer"][tipo][-_CALIB_MAX:]

    elapsed = _time.time() - _calib["inicio"]
    if elapsed >= _CALIB_DURACION:
        _calib["activa"] = False

    return {"status": "ok", "elapsed": round(elapsed, 1)}

@app.get("/calibracion/estado")
def estado_calibracion():
    """El panel admin consulta esto periódicamente para mostrar los datos en tiempo real."""
    elapsed = (_time.time() - _calib["inicio"]) if _calib["inicio"] else 0
    return {
        "activa":  _calib["activa"],
        "elapsed": round(elapsed, 1),
        "buffer":  _calib["buffer"]
    }
...
def horas_sesion_endpoint(sesion_id: int):
    try:
        ...
        cursor.execute("""
            ...
            WHERE hs.sesion_id=%s GROUP BY hs.id ORDER BY hs.numero_hora
        """, (sesion_id,))
        rows = cursor.fetchall()
        cursor.close(); conn.close()
        result = []
...
def interrupciones_por_sesion(sesion_id: int):
    try:
        ...
        cursor.execute("""
            ...
            FROM horas_sesion hs WHERE hs.sesion_id=%s
        """, (sesion_id,))
        horas_info = cursor.fetchall()
        hora_orden_map = {h["hs_id"]: h["hora_orden"] for h in horas_info}
        cursor.close(); conn.close()
        result = []
...
def senales_completas(interrupcion_id: int):
    try:
        ...
        for tipo, data in raw.items():
            ts, vs = data["timestamps"], data["valores"]
            if tipo.lower() == "ecg":
...
def _construir_resp_desde_streaming(ts_flujo, vs_flujo, ts_accz, vs_accz):
    def interpolar_uniforme(ts, vs, n=500):
        if len(ts) < 2: return ts[:], vs[:]
        t0, t1 = ts[0], ts[-1]
        if t0 == t1: return ts[:], vs[:]
        paso = (t1 - t0) / (n - 1)
        ts_n = [int(t0 + i * paso) for i in range(n)]
        vs_n = []
        j = 0
        for t in ts_n:
            while j < len(ts) - 2 and ts[j + 1] < t: j += 1
            t_a = ts[j]; t_b = ts[min(j+1, len(ts)-1)]
            v_a = vs[j]; v_b = vs[min(j+1, len(vs)-1)]
    ...
    def normalizar_al_rango(valores, ref_min, ref_max):
        ...
        v_min = min(valores); v_max = max(valores)
    ...
    if tiene_flujo and tiene_accz:
        ...
        f_min = min(vs_flujo_i); f_max = max(vs_flujo_i)
...
def crear_usuario(data: UsuarioModel, request: Request):
    ...
    cursor.execute("INSERT INTO usuarios (usuario, contrasena) VALUES (%s,%s)", (data.usuario, hashed))
    conn.commit(); cursor.close(); conn.close()
    return {"status": "success"}
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tab { padding: 12px 22px; cursor: pointer; font-weight: bold; font-size: 13px; color: #5A7A8A; border-bottom: 3px solid transparent; white-space: nowrap; }
        ...
        /* ── Modals ── */
        .modal-bg { display: none; position: fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,.3); z-index:100; justify-content:center; align-items:center; }
        ...
        .visor-panel h3 { font-size: 13px; color: #5A7A8A; margin-bottom: 12px; text-transform: uppercase; letter-spacing: .5px; }
        ...
        .interr-item { padding: 10px 12px; border-radius: 6px; cursor: pointer; margin-bottom: 6px; background: white; border: 1px solid #D4E8F3; transition: all .15s; }
        ...
        .chart-card h4 { font-size: 12px; color: #5A7A8A; text-transform: uppercase; letter-spacing: .5px; margin-bottom: 10px; }
        ...
        /* ══════════════════════════════════════
           CALIBRACIÓN
        ══════════════════════════════════════ */
        .calib-wrapper { max-width: 1100px; }
        .calib-hero { background: linear-gradient(135deg, #2C4A5A 0%, #3E6B80 100%); border-radius: 12px; padding: 24px 28px; color: white; margin-bottom: 22px; display: flex; align-items: center; gap: 20px; }
        .calib-hero-icon { font-size: 48px; }
        .calib-hero h2 { font-size: 20px; margin-bottom: 4px; }
        .calib-hero p { font-size: 13px; opacity: .8; line-height: 1.5; }
        .calib-layout { display: grid; grid-template-columns: 260px 1fr; gap: 20px; }
        .calib-sidebar { display: flex; flex-direction: column; gap: 14px; }
        .calib-card { background: #EEF5FB; border: 1px solid #D4E8F3; border-radius: 10px; padding: 18px; }
        .calib-timer-display { text-align: center; }
        .calib-countdown { font-size: 56px; font-weight: bold; color: #2C4A5A; line-height: 1; font-family: 'Courier New', monospace; }
        .calib-countdown.running { color: #2E7D52; }
        .calib-countdown.done    { color: #7AAFC5; }
        .calib-label { font-size: 11px; color: #5A7A8A; text-align: center; margin-top: 4px; text-transform: uppercase; letter-spacing: .5px; }
        .calib-progress-wrap { margin: 14px 0 10px; }
        .calib-progress-bar  { background: #D4E8F3; border-radius: 10px; height: 8px; overflow: hidden; }
        .calib-progress-fill { background: linear-gradient(90deg, #7AAFC5, #4A9E6B); height: 100%; border-radius: 10px; transition: width .8s linear; width: 0%; }
        .calib-status-pill { text-align: center; padding: 6px 14px; border-radius: 20px; font-size: 12px; font-weight: bold; background: #D4E8F3; color: #2C4A5A; margin-bottom: 12px; }
        .calib-status-pill.running { background: #EEF8F2; color: #2E7D52; }
        .calib-status-pill.done    { background: #E3F2FA; color: #2C6080; }
        .calib-status-pill.error   { background: #FFF0EE; color: #A02020; }
        .btn-calib { width: 100%; padding: 11px; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: bold; margin-bottom: 8px; transition: opacity .2s; }
        .btn-calib:disabled { opacity: .4; cursor: not-allowed; }
        .btn-calib-start { background: #4A9E6B; color: white; }
        .btn-calib-stop  { background: #D65C5C; color: white; }
        .sensor-status-list { display: flex; flex-direction: column; gap: 8px; }
        .sensor-row { display: flex; align-items: center; justify-content: space-between; background: white; border: 1px solid #D4E8F3; border-radius: 6px; padding: 8px 12px; }
        .sensor-row-left { display: flex; align-items: center; gap: 8px; font-size: 13px; font-weight: bold; color: #2C4A5A; }
        .sensor-dot { font-size: 12px; color: #ccc; transition: color .4s; }
        .sensor-dot.green  { color: #2E7D52; }
        .sensor-dot.yellow { color: #B07020; }
        .sensor-dot.red    { color: #A02020; }
        .sensor-val { font-size: 11px; color: #5A7A8A; font-family: monospace; }
        .calib-url-card { background: #2C4A5A; color: white; border-radius: 8px; padding: 12px 14px; font-size: 12px; line-height: 1.7; }
        .calib-url-card code { display: block; background: rgba(255,255,255,.1); border-radius: 4px; padding: 6px 10px; margin-top: 6px; font-size: 11px; word-break: break-all; }
        .calib-charts-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
        .calib-result-card { background: white; border: 2px solid #D4E8F3; border-radius: 10px; padding: 20px; margin-top: 16px; }
        .calib-result-card h3 { margin-bottom: 14px; color: #2C4A5A; }
        .calib-result-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; }
        .calib-result-item { text-align: center; padding: 12px 8px; border-radius: 8px; border: 1px solid #D4E8F3; }
        .calib-result-item .r-icon  { font-size: 24px; margin-bottom: 4px; }
        .calib-result-item .r-label { font-size: 11px; color: #5A7A8A; }
        .calib-result-item .r-val   { font-size: 14px; font-weight: bold; color: #2C4A5A; margin-top: 2px; }
        .calib-result-item.ok   { background: #EEF8F2; border-color: #A8D8B8; }
        .calib-result-item.warn { background: #FFF8EC; border-color: #F0C060; }
        .calib-result-item.bad  { background: #FFF0EE; border-color: #F0A0A0; }
    </style>
</head>
<body>
...
<div class="tabs">
    ...
    <div class="tab"        onclick="cambiarTab('pruebas',    event)">📋 Pruebas</div>
    <div class="tab"        onclick="cambiarTab('calibracion',event)">🔧 Calibración</div>
    <div class="tab"        onclick="cambiarTab('monitoreo',  event)">📊 Monitoreo</div>
...
<div class="content">
    ...
    <!-- ══ PRUEBAS (visor de señales) ══ -->
    <div id="sec-pruebas" class="section">
    ...
    <!-- ══ CALIBRACIÓN ══ -->
    <div id="sec-calibracion" class="section">
        <div class="calib-wrapper">
            <div class="calib-hero">
                <div class="calib-hero-icon">🔧</div>
                <div>
                    <h2>Calibración de Sensores</h2>
                    <p>Realiza una prueba de 60 segundos para verificar el correcto funcionamiento de todos los sensores<br>
                    antes de iniciar una sesión de monitoreo de apnea. El ESP32 debe enviar datos a <strong>POST /calibracion/datos</strong>.</p>
                </div>
            </div>
            ...
            <div class="calib-layout">
                ...
                <div class="calib-sidebar">
                    <!-- Timer -->
                    <div class="calib-card calib-timer-display">
                        <div class="calib-countdown" id="calib-countdown">60</div>
                        <div class="calib-label">segundos restantes</div>
                        <div class="calib-progress-wrap">
                            <div class="calib-progress-bar">
                                <div class="calib-progress-fill" id="calib-progress-fill"></div>
                            </div>
                        </div>
                        <div class="calib-status-pill" id="calib-status-pill">🔵 Listo para calibrar</div>
                        <button class="btn-calib btn-calib-start" id="btn-calib-start" onclick="iniciarCalibracion()">▶ Iniciar calibración</button>
                        <button class="btn-calib btn-calib-stop"  id="btn-calib-stop"  onclick="detenerCalibracion()" disabled>⏹ Detener</button>
                    </div>

                    <!-- Sensor status -->
                    <div class="calib-card">
                        <div style="font-size:11px;color:#5A7A8A;font-weight:bold;text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px;">Estado de sensores</div>
                        <div class="sensor-status-list">
                            <div class="sensor-row">
                                <div class="sensor-row-left">
                                    <span class="sensor-dot" id="dot-ecg">●</span>
                                    <span>❤️ ECG</span>
                                </div>
                                <span class="sensor-val" id="lbl-ecg">--</span>
                            </div>
                            <div class="sensor-row">
                                <div class="sensor-row-left">
                                    <span class="sensor-dot" id="dot-spo2">●</span>
                                    <span>🩸 SpO₂</span>
                                </div>
                                <span class="sensor-val" id="lbl-spo2">--</span>
                            </div>
                            <div class="sensor-row">
                                <div class="sensor-row-left">
                                    <span class="sensor-dot" id="dot-acce_z">●</span>
                                    <span>🔵 Acel. Z</span>
                                </div>
                                <span class="sensor-val" id="lbl-acce_z">--</span>
                            </div>
                            <div class="sensor-row">
                                <div class="sensor-row-left">
                                    <span class="sensor-dot" id="dot-flujo">●</span>
                                    <span>💨 Flujo</span>
                                </div>
                                <span class="sensor-val" id="lbl-flujo">--</span>
                            </div>
                        </div>
                    </div>

                    <!-- URL info para ESP32 -->
                    <div class="calib-url-card">
                        <strong>📡 Endpoint para ESP32</strong>
                        <code id="calib-endpoint-url">POST [tu-servidor]/calibracion/datos</code>
                        <div style="margin-top:8px;font-size:11px;opacity:.8;">
                            Body JSON: <br>{ spo2, ecg, acce_z, flujo }
                        </div>
                    </div>
                </div>
                ...
                <!-- Charts area -->
                <div>
                    <div class="calib-charts-grid">
                        <div class="chart-card">
                            <h4>❤️ ECG</h4>
                            <canvas id="calib-chart-ecg" height="130"></canvas>
                        </div>
                        <div class="chart-card">
                            <h4>🩸 SpO₂</h4>
                            <canvas id="calib-chart-spo2" height="130"></canvas>
                        </div>
                        <div class="chart-card">
                            <h4>🔵 Aceleración Z</h4>
                            <canvas id="calib-chart-acce_z" height="130"></canvas>
                        </div>
                        <div class="chart-card">
                            <h4>💨 Flujo Respiratorio</h4>
                            <canvas id="calib-chart-flujo" height="130"></canvas>
                        </div>
                    </div>

                    <!-- Result summary (oculto hasta que termina) -->
                    <div class="calib-result-card" id="calib-result" style="display:none;">
                        <h3 id="calib-result-title">Resultado de calibración</h3>
                        <div class="calib-result-grid" id="calib-result-grid"></div>
                    </div>
                </div>
            </div>
        </div>
...
const SIGNAL_CONFIG = {
    ...
    frecuencia_respiratoria: { label:'Flujo Respiratorio', color:'#4A9E6B', bg:'rgba(74,158,107,0.12)', unit:'ADC', emoji:'🌬️' },
};
...
function cambiarTab(tab, event) {
    ...
    if (tab === 'pruebas')     iniciarVisor();
    if (tab === 'calibracion') iniciarPanelCalibracion();
}
...
// ════════════════════════════════════════════════
const CALIB_COLORS = {
    ecg:    { color:'#E05C5C', bg:'rgba(224,92,92,0.12)' },
    spo2:   { color:'#5C9AE0', bg:'rgba(92,154,224,0.12)' },
    acce_z: { color:'#5CBE80', bg:'rgba(92,190,128,0.12)' },
    flujo:  { color:'#E0A55C', bg:'rgba(224,165,92,0.12)' },
};

let _calibTimer = null;   // setInterval para el countdown
let _calibPoll  = null;   // setInterval para el polling
let _calibSeg   = 0;
let _calibCharts = {};

function iniciarPanelCalibracion() {
    // Mostrar la URL del servidor actual
    const base = window.location.origin;
    document.getElementById('calib-endpoint-url').textContent =
        'POST ' + base + '/calibracion/datos';
}

function crearChartsCalibracion() {
    // Destruir instancias previas
    Object.values(_calibCharts).forEach(c => c.destroy());
    _calibCharts = {};

    for (const tipo of ['ecg','spo2','acce_z','flujo']) {
        const ctx = document.getElementById('calib-chart-' + tipo).getContext('2d');
        const cfg = CALIB_COLORS[tipo];
        _calibCharts[tipo] = new Chart(ctx, {
            type: 'line',
            data: {
                labels: [],
                datasets: [{
            ...
            options: {
                ...
                scales: {
                    x: { ticks: { maxTicksLimit: 6, font:{ size:9 }, color:'#5A7A8A' }, grid:{ color:'#EEF5FB' } },
                    y: { ticks: { font:{ size:9 }, color:'#5A7A8A' }, grid:{ color:'#EEF5FB' } }
                }
            }
        });
    }
...
async function iniciarCalibracion() {
    try {
        const res = await fetch('/calibracion/iniciar', { method: 'POST' });
        if (!res.ok) { mostrarToast('❌ Error al iniciar calibración'); return; }
    } catch(e) {
        mostrarToast('❌ No se pudo conectar con el servidor'); return;
    }
    ...
    // UI: estado inicial
    const countdown = document.getElementById('calib-countdown');
    countdown.textContent = '60';
    countdown.className   = 'calib-countdown running';

    const pill = document.getElementById('calib-status-pill');
    pill.textContent = '🟡 Calibrando... esperando datos del ESP32';
    pill.className   = 'calib-status-pill running';

    document.getElementById('calib-progress-fill').style.width = '0%';
    document.getElementById('btn-calib-start').disabled = true;
    document.getElementById('btn-calib-stop').disabled  = false;

    // Reset dots
    for (const tipo of ['ecg','spo2','acce_z','flujo']) {
        const dot = document.getElementById('dot-' + tipo);
        dot.className = 'sensor-dot';
        document.getElementById('lbl-' + tipo).textContent = '--';
    }

    crearChartsCalibracion();

    // Countdown
    _calibTimer = setInterval(() => {
        _calibSeg++;
        const remaining = 60 - _calibSeg;
        document.getElementById('calib-countdown').textContent = remaining;
        document.getElementById('calib-progress-fill').style.width = (_calibSeg / 60 * 100) + '%';
        if (remaining <= 0) finalizarCalibracion(true);
    }, 1000);

    // Polling de datos
    _calibPoll = setInterval(pollCalibracion, 800);
}

async function pollCalibracion() {
    try {
        const res  = await fetch('/calibracion/estado');
        const data = await res.json();
        actualizarChartsCalibracion(data.buffer);
        actualizarStatusSensores(data.buffer);
        // Si el servidor la detuvo (tiempo cumplido)
        if (!data.activa && _calibSeg >= 3) {
            finalizarCalibracion(true);
        }
    } catch(e) {
        console.error('Poll calibración error:', e);
    }
}

function actualizarChartsCalibracion(buffer) {
    for (const tipo of ['ecg','spo2','acce_z','flujo']) {
        const chart = _calibCharts[tipo];
        if (!chart) continue;
        const datos = buffer[tipo] || [];
        if (!datos.length) continue;
        const t0 = datos[0].t;
        chart.data.labels            = datos.map(d => ((d.t - t0) / 1000).toFixed(1) + 's');
        chart.data.datasets[0].data  = datos.map(d => d.v);
        chart.update('none');
    }
}

function actualizarStatusSensores(buffer) {
    for (const tipo of ['ecg','spo2','acce_z','flujo']) {
        const datos = buffer[tipo] || [];
        const dot   = document.getElementById('dot-' + tipo);
        ...
        lbl.textContent = last.v.toFixed(2);
        // Comparar con tiempo actual (epoch ms)
        const age = Date.now() - last.t;
        if (age < 3000)       dot.className = 'sensor-dot green';
        else if (age < 10000) dot.className = 'sensor-dot yellow';
        else                   dot.className = 'sensor-dot red';
    }
}

async function detenerCalibracion() {
    await fetch('/calibracion/detener', { method: 'POST' }).catch(() => {});
    finalizarCalibracion(false);
}

function finalizarCalibracion(completada) {
    clearInterval(_calibTimer);
    clearInterval(_calibPoll);
    _calibTimer = null;
    _calibPoll  = null;

    document.getElementById('btn-calib-start').disabled = false;
    document.getElementById('btn-calib-stop').disabled  = true;

    const countdown = document.getElementById('calib-countdown');
    countdown.textContent = completada ? '✓' : '—';
    countdown.className   = 'calib-countdown done';

    const pill = document.getElementById('calib-status-pill');
    if (completada) {
        pill.textContent = '✅ Calibración completada';
        pill.className   = 'calib-status-pill done';
    } else {
        pill.textContent = '🔴 Calibración detenida manualmente';
        pill.className   = 'calib-status-pill error';
    }

    document.getElementById('calib-progress-fill').style.width = completada ? '100%' : document.getElementById('calib-progress-fill').style.width;

    // Mostrar resumen
    mostrarResumenCalibracion();
}

async function mostrarResumenCalibracion() {
    let buffer = {};
    try {
        const res  = await fetch('/calibracion/estado');
        const data = await res.json();
    ...
    const TIPOS = [
        { tipo:'ecg',    icon:'❤️',  label:'ECG',           unidad:'mV'   },
        { tipo:'spo2',   icon:'🩸',  label:'SpO₂',          unidad:'%'    },
        { tipo:'acce_z', icon:'🔵',  label:'Aceleración Z', unidad:'m/s²' },
        { tipo:'flujo',  icon:'💨',  label:'Flujo',         unidad:'ADC'  },
    ];

    const grid = document.getElementById('calib-result-grid');
    grid.innerHTML = TIPOS.map(({ tipo, icon, label, unidad }) => {
        const datos = buffer[tipo] || [];
        const n = datos.length;
        let cls = 'bad', info = 'Sin datos';
        if (n > 0) {
            const vals = datos.map(d => d.v);
            const avg  = (vals.reduce((a,b) => a+b, 0) / n).toFixed(2);
            info = `${avg} ${unidad} · ${n} muestras`;
            cls  = n > 30 ? 'ok' : 'warn';
        }
        return `<div class="calib-result-item ${cls}">
            <div class="r-icon">${icon}</div>
            <div class="r-label">${label}</div>
            <div class="r-val" style="font-size:12px;">${info}</div>
        </div>`;
    }).join('');

    document.getElementById('calib-result-title').textContent =
        _calibSeg >= 60 ? '✅ Resultado — Calibración de 60s' : '📊 Resultado parcial';
...
function renderInterrList(interrupciones) {
    ...
    if (!interrupciones.length) {
        cont.innerHTML = '<p style="font-size:12px;color:#5A7A8A;text-align:center;padding:20px 0;">No hay apneas en esta sesión</p>';
        return;
    }
    cont.innerHTML = interrupciones.map((i, idx) => {
        const spo2Class  = i.spo2 < 90 ? 'badge-crit' : i.spo2 < 95 ? 'badge-warn' : 'badge-ok';
        const tieneSenales = i.total_senales > 0;
        const numApnea   = i.numero_consecutivo || (idx + 1);
...
function renderTabsSignal(tipos, data) {
    ...
    tabsCont.innerHTML = tiposOrdenados.map((tipo, idx) => {
        ...
        return `<span class="tab-signal ${idx===0?'active':''}" data-tipo="${tipo}">
            ${cfg.emoji} ${cfg.label} <span style="opacity:.7;font-size:10px;">(${n})</span>
        </span>`;
    }).join('');
...
function detectarYFiltrarRuido(timestamps, valores) {
    ...
    const b0=1, b1=-2*cosOmega, b2=1, a1=-2*r*cosOmega, a2=r*r;
...
function confirmarEliminarPaciente(id, nombre) {
    abrirConfirm(
        '🗑️ Eliminar paciente',
        `Se eliminarán todos los datos de "${nombre}": sesiones, apneas y señales. Esta acción no se puede deshacer.`,
        async () => {
