...
def dashboard():
    ...
    <html>
    <head>
        <title>Base de Datos - Sensores</title>
        <style>
    ...
    <body>
        ...
        <script>
            ...
            function mostrar(datos) {
                document.getElementById('total').innerText = `Total registros: ${datos.length}`;
                const cuerpo = document.getElementById('cuerpo');
                cuerpo.innerHTML = '';
                datos.forEach(d => {
                    cuerpo.innerHTML += `<tr>
                        <td>${d.id}</td><td>${d.tiempo_ms}</td>
                        <td>${d.ecg}</td><td>${d.spo2}</td>
                        <td>${d.acc_x}</td><td>${d.acc_y}</td><td>${d.acc_z}</td>
                        <td>${d.giro_x}</td><td>${d.giro_y}</td><td>${d.giro_z}</td>
                        <td>${d.flujo_respiratorio}</td><td>${d.fecha_registro}</td>
                    </tr>`;
                });
            }
            cargarDatos();
